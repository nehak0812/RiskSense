    ...
    if (!signal) {
      ...
      signal = await prisma.signal.create({
        data: {
          ...
          url: result.url,
          publishedAt: result.published_date && !isNaN(new Date(result.published_date).getTime()) ? new Date(result.published_date) : new Date(),
          title: result.title,
          summary: enrichment.summary,
          body: enrichment.whyItMatters || result.content || "",
        ...
        if (!exists) {
          ...
          await prisma.signal.create({
            data: {
              ...
              title: result.title,
              summary: enrichment.summary,
              body: enrichment.whyItMatters || result.content || result.snippet || "",
              domain: enrichment.domain,
              sentiment: enrichment.sentiment,
              impact: enrichment.impact,
              entities: JSON.stringify(enrichment.entities || []),
